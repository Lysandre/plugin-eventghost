...
class Eventghost extends eqLogic {
    /******************************* Attributs *******************************/ 
    ...
    */
    ...
    public function initStackData() {
        nodejs::pushUpdate('Eventghost::initStackDataEqLogic', array('plugin' => 'Eventghost', 'action' => 'saveStack', 'callback' => 'displayEqLogic'));
    }
    ...
    public function stackData($params) {
        ...
        nodejs::pushUpdate('Eventghost::stackDataEqLogic', $paramsArray);
    }
...
class EventghostCmd extends cmd {
    /******************************* Attributs *******************************/ 
    /* Ajouter ici toutes vos variables propre à votre classe */
    ...
    */
    public function preSave() {
      /*
        if ($this->getConfiguration('name') == '') {
            throw new Exception('Event ne peut etre vide');
        }
        */
    }
    public function execute($_options = array()) {
        ...
        */
        ...
        /* URL GHOST http://eventghostip:port/?myevent */
        $url = trim($this->getEqLogic()->getConfiguration('eventghostip')).':'.trim($this->getEqLogic()->getConfiguration('eventghostport')). '?' . trim($this->getConfiguration('event'));
        
        log::add('Eventghost','debug','Appel de l evenement :'.$url);
        $ch = curl_init($url);
